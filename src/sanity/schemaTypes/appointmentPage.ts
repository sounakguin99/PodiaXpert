...
export const appointmentPage = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      options: { collapsible: true, collapsed: true },
      fields: [
        defineField({ name: 'title', type: 'string', title: 'Banner Title' }),
        defineField({ name: 'description', type: 'text', title: 'Banner Description' }),
        defineField({ name: 'subtitle', type: 'string', title: 'Banner Subtitle / Highlight' })
      ]
    }),
  ...
  preview: {
    ...
    prepare({ subtitle }) {
      return {
        title: 'Book Appointment Page',
        subtitle: subtitle ? `Hero: ${subtitle}` : 'Edit appointment page content'
      }
    }
  }
})
