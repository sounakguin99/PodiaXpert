export default function Expertise({ data }: { data?: any }) {
  ...
  const steps = data?.expertiseItems?.map((item: any, index: number) => ({
    number: (index + 1).toString(),
    title: item.title,
    desc: item.description,
    color: index % 3 === 0 ? "text-red-500" : index % 3 === 1 ? "text-emerald-500" : "text-blue-500"
  })) || defaultSteps;
    ...
    <section className="bg-[#1a1a1a] py-24">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <p className="text-emerald-500 font-bold tracking-wider text-sm uppercase mb-4">
          {data?.tagline || "Our Expertise"}
        </p>
        <h2 className="text-3xl md:text-4xl font-bold text-white mb-16">
          {data?.heading || "Comprehensive Foot Care"}
        </h2>
